...
export class MapComponent implements OnInit, OnDestroy {
  ...
  watchMapRedirect() {
    this._redirectToPolygonSubject
      .watch()
      .subscribe(shapeId => this.redirectToShape(shapeId));
  }
      ...
      for (let i = 0; i < shape.coordinates.length; i++) {
        bounds.extend(
          new google.maps.LatLng(
            shape.coordinates[i].lat,
            shape.coordinates[i].lng
          )
        );
      }

      this._router.navigate(["/zone-detail/" + shape.id], {
        queryParams: {
          zoom: this.getZoomByBounds(this.map, bounds),
          lat: bounds
            .getCenter()
            .lat()
            .toFixed(5),
    ...
    for (let zoom = MAX_ZOOM; zoom >= MIN_ZOOM; --zoom) {
      if (
        worldCoordWidth * (1 << zoom) + 2 * FIT_PAD <
          this.mapElement.nativeElement.clientWidth &&
        worldCoordHeight * (1 << zoom) + 2 * FIT_PAD <
          this.mapElement.nativeElement.clientHeight
      )
        return zoom;
    }
    return 0;
  ...
  watchShapeUpdate() {
    this.mapShapeConnection = new signalR.HubConnectionBuilder()
      .withUrl(WsUrls.mapShape)
      .build();

    this.mapShapeConnection.start();

    this.mapShapeConnection.on(
      "MapShapeChange",
      (type: string, data: MapShapeItem) => {
        if (type === "update") {
          this._ngRedux.dispatch({
            type: MapShapeAction.update,
            payload: [data]
          ...
          if (!shape) {
            ...
            this.mapShapes = this.mapShapes.map(s =>
              s.id != data.id ? s : data
            );
          }
        } else if (type === "delete") {
          this._ngRedux.dispatch({
            type: MapShapeAction.remove,
            payload: [data]
          } as MapShapeDispatch);
    ...
    if (this.isBrowser) {
      const mapSetup = {
        center: new google.maps.LatLng(
          this.mapData.position.lat,
          this.mapData.position.lng
        ),
  ...
  watchMapData() {
    this._activatedRoute.queryParams.subscribe(params => {
      ...
      if (this.isBrowser && this.map) {
        ...
        this.map.setCenter(
          new google.maps.LatLng(
            Number(this.mapParams["lat"]),
            Number(this.mapParams["lng"])
          )
        );
      }
    });
